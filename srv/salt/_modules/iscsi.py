...
class CephIscsiConfig(object):
    ...
    def add_target_auth(self, target_iqn,
        ...
        if 'auth' in target_config.keys():
            ...
            # TODO also catch cases where auth is set on one TPG but not on another
    ...
    def add_disk(self, target_iqn, pool, image, wwn, tpg_lun_id):
        ...
        if disk_id in self.config['disks']:
            ...
            return  # XXX need to check for TPG export?
        ...
        for disk_k, disk_v in self.config['targets'][target_iqn]['disks'].items():
            if disk_k == disk_id:
                ...
                return
            elif disk_v['lun_id'] == tpg_lun_id:
                raise Exception("disks {} and {} have clashing LUN {}".format(disk_id, disk, tpg_lun_id))
...
def validate(lio_root):
    ...
    for target in lio_root.targets:
        ...
        tpg_acls_prev = None
        for tpg in target.tpgs:
            tpg_auth = {tpg.get_attribute('authentication'),
                        tpg.chap_userid,
                        tpg.chap_password,
            ...
            elif tpg_auth_prev != tpg.node_acls:
                raise Exception(
            ...
            for lun in tpg.luns:
                ...
                if targets_by_disk[disk_id]['target_iqn'] != disk_details['target_iqn']:
                    raise Exception(
                        ...
                        'export to proceed.'.format(disk_id,
                        targets_by_disk[disk_id]['target_iqn'],
